...
// Update user
user.patch("/me", updateUserDoc, validator("json", UserUpdateSchema, validationHook), async (c) => {
  try {
    ...
    return c.json(successResponse(response, "User updated successfully"), HttpStatusCodes.OK);
  } catch (error) {
    if (error instanceof APIError) {
      return c.json(
        errorResponse(error.body?.code ?? "AUTH_ERROR", error.body?.message ?? error.message),
