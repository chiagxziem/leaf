...
import {
  ...
  createRootFolderDoc,
  deleteFolderDoc,
  getFolderChildrenDoc,
  getFolderWithItemsDoc,
  moveFolderDoc,
  updateFolderDoc,
} from "./folder.docs";

const folderRouter = createRouter().use(authed);
  ...
  validator(
    ...
    z.object({
      folderId: z
        .uuid()
        .optional()
        .describe("Folder ID. If not provided, returns the root folder."),
    }),
    validationHook,
  ),
  async (c) => {
...
folderRouter.post(
  "/",
  createFolderDoc,
  validator("json", FolderInsertSchema, validationHook),
  async (c) => {
    const user = c.get("user");
...
folderRouter.put(
  ...
  updateFolderDoc,
  validator("param", z.object({ id: z.uuid() }), validationHook),
  validator("json", FolderUpdateSchema, validationHook),
  async (c) => {
    const user = c.get("user");
    const { id } = c.req.valid("param");
    const folderData = c.req.valid("json");
...
folderRouter.get(
  "/:id/children",
  getFolderChildrenDoc,
  validator("param", z.object({ id: z.string().uuid() }), validationHook),
  async (c) => {
    const user = c.get("user");
    const { id } = c.req.valid("param");
...
// Create root folder
folderRouter.post("/root", createRootFolderDoc, async (c) => {
  const user = c.get("user");
